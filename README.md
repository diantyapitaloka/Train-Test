...
- Introduction to PCA: There are four dimensions, Iris is the perfect candidate for Principal Component Analysis (PCA) to reduce the data to 2D. This is teaches you how to compress information while retaining the variance necessary to still distinguish the three species.
...
- Impact of Data Shuffling: The Iris dataset in SKLearn is often sorted by class (all Setosa first, then Versicolor). Henc, this provides a "gotcha" moment for students: if they don't shuffle their data before splitting, their training set might be contain zero examples of one species, causing the model to fail spectacularly.
- Confusion Matrix Interpretation: With three species, the Confusion Matrix becomes a $3 \times 3$ grid. In addition, the teacher student how to read "off-diagonal" and also errors specifically observing that to the models rarely confuses Setosa with others, but often confuses the other two.
- Hyperparameter Tuning with GridSearch: Because the dataset is so tiny, running a GridSearchCV or RandomizedSearchCV takes milliseconds. The ideal environment to learn how to tune "K" in KNN and "C" and "Gamma" in an SVM without waiting for hours of compute time.
- Probabilistic Outputs: Instead of just predicting a label, Iris is great for learning about predict_proba. Hence, you can see how a models will be 99% sure about a Setosa, but like only 55% sure when deciding between a Versicolor and a Virginica.
