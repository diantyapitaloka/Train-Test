...
- Confusion Matrix Interpretation: With three species, the Confusion Matrix becomes a $3 \times 3$ grid. This teaches students how to read "off-diagonal" errors—specifically observing that the models rarely confuses Setosa with others, but often confuses the other two.
...
- Probabilistic Outputs: Instead of just predicting a label, Iris is great for learning about predict_proba. You can see how a model might be 99% sure about a Setosa, but only 55% sure when deciding between a Versicolor and a Virginica.
